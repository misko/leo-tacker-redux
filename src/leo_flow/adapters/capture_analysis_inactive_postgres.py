"""PostgreSQL admission gate for capture-first deferred analysis."""
...
"""
_INACTIVE_SQL = """
SELECT public.capture_analysis_inactive() AS ready
"""


class PostgresCaptureAnalysisInactiveGate:
    """Allow pending backlog only when no analysis lease is currently active."""
    ...
    def ready(self) -> bool:
        with psycopg.connect(
            ...
            row = connection.execute(_INACTIVE_SQL).fetchone()
        if row is None or not isinstance(row[0], bool):
            raise RuntimeError("capture-analysis inactive gate returned no decision")
        return row[0]
