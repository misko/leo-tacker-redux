"""Durable local SQLite journal for the finite Gauss V5 campaign."""
...
MAX_CAMPAIGN_STATE_BYTES = 16 * 1024 * 1024


class SQLiteCampaignJournal:
    """One digest-bound campaign state with full-sync compare-and-swap updates."""

    def __init__(
        self, database_path: Path, *, now_utc_ns: Callable[[], int] = time.time_ns
    ) -> None:
        if not database_path.is_absolute() or ".." in database_path.parts:
            raise ValueError("campaign journal path must be absolute and normalized")
    ...
    def initialize(self, definition: CampaignDefinition) -> CampaignJournalState:
        initial = CampaignJournalState(definition.digest)
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT * FROM campaign_journal WHERE campaign_id = ?",
    ...
    def compare_and_swap(
        ...
        with self._connect() as connection:
            ...
            if cursor.rowcount != 1:
                raise RuntimeError("campaign journal revision or definition changed")
        return replacement
    ...
    def _decode_row(
        ...
        try:
            ...
            raise RuntimeError(
                "campaign journal failed integrity validation"
            ) from error


def _encode_state(state: CampaignJournalState) -> bytes:
    ...
    if len(payload) > MAX_CAMPAIGN_STATE_BYTES:
        raise RuntimeError("campaign state exceeds the size limit")
    return payload
...
def _decode_state(
    ...
    root = _mapping(json.loads(payload), "campaign state")
    if set(root) != {
        "definition_digest",
        "revision",
        "records",
    }:
        raise ValueError("campaign state fields differ")
    digest = _digest(root["definition_digest"])
    if digest != definition.digest:
        raise ValueError("campaign definition digest differs")
    ...
    if (
        ...
        raise ValueError("campaign state values are invalid")
    records: list[CampaignUnitRecord] = []
    for value in records_value:
        item = _mapping(value, "campaign record")
        if set(item) != {
            "success_index",
            "slot_index",
            "retry_index",
            ...
            raise ValueError("campaign record fields differ")
        integers = tuple(
            item[name]
            for name in (
                "success_index",
        ...
        if any(
            ...
            raise ValueError("campaign record integers are invalid")
        unit = build_campaign_unit(
            definition,
            success_index=item["success_index"],
            slot_index=item["slot_index"],
            retry_index=item["retry_index"],
            requested_start_utc_ns=UtcNs(item["requested_start_utc_ns"]),
        )
        if str(unit.digest) != item["unit_digest"]:
            raise ValueError("campaign unit digest differs")
        ...
        if snapshot is not None and snapshot.definition != unit.batch:
            raise ValueError("campaign snapshot identifies another unit")
        ...
        if receipt is not None and receipt.batch_id != unit.batch.batch_id:
            raise ValueError("analysis receipt identifies another unit")
        records.append(
            CampaignUnitRecord(
                unit,
                CampaignUnitPhase(item["phase"]),
                item["capture_invocations"],
                item["analysis_invocations"],
                snapshot,
                receipt,
            )
        )
    state = CampaignJournalState(digest, tuple(records), revision)
    if _encode_state(state) != payload:
        raise ValueError("campaign state is not canonical")
    return state
...
def _analysis_success(value: Mapping[str, Any]) -> CampaignAnalysisSuccess:
    ...
    if set(result) != {"artifact_id", "digest", "schema"}:
        raise ValueError("analysis result fields differ")
    feature = _mapping(value["projected_feature_set_ref"], "feature ref")
    if set(feature) != {"feature_set_id", "analysis_run_id", "bundle_ref"}:
        raise ValueError("feature ref fields differ")
    schema_value = result["schema"]
    schema = None if schema_value is None else _schema(_mapping(schema_value, "schema"))
    return CampaignAnalysisSuccess(
        RecordingId(_string(value["recording_id"])),
        JobId(_string(value["analysis_job_id"])),
        ArtifactRef(
            _string(result["artifact_id"]), _structured_digest(result["digest"]), schema
        ),
