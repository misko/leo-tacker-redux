...
def _validate_object_ref(
    ref: Mapping[str, Any], location: str, roots: set[str]
) -> None:
    _require(
        set(ref) == {"root_id", "relative_path", "sha256", "bytes"},
    ...
    _require(
        isinstance(ref["bytes"], int) and ref["bytes"] >= 0,
...
def validate_manifest(
    manifest: Mapping[str, Any], *, promotion_gate: bool = False
) -> None:
    required_top = {
        "schema",
        "corpus_id",
    ...
    _require(
        manifest["canonicalization"] == CANONICALIZATION, "unsupported canonicalization"
    )
    _require(
        manifest["purpose"] in {"development", "locked_test"},
    ...
    _require(
        isinstance(root_items, list) and root_items, "storage_roots must be non-empty"
    )
    root_ids: set[str] = set()
    for index, root in enumerate(root_items):
        _require(
            set(root) == {"root_id", "description"},
    ...
    _require(
        policy.get("unit") == "split_group_id", "split unit must be split_group_id"
    )
    _require(
        policy.get("assignment") == "explicit_frozen",
        "split assignment must be explicit_frozen",
    )
    _require(
        policy.get("time_order") == ["train", "validation", "locked_test"],
        "time_order must be train, validation, locked_test",
    )
    _require(
        policy.get("half_open_intervals") is True, "split intervals must be half-open"
    )
    truth_policy = manifest["truth_policy"]
    _require(
        truth_policy.get("unlabeled_is_negative") is False,
        "unlabeled sky must never be treated as negative",
    )
    _require(
        truth_policy.get("tle_only_is_ground_truth") is False,
    ...
    required_member = {
        ...
        "recording_id",
        "partition",
        "legacy_manifest_created_utc_ns",
        "grouping",
        "split_group_id",
        "coverage",
        "truth",
        ...
        "source_manifest_ref",
        "payload_set",
        "legacy_analysis_report_ref",
        "legacy_followup_report_ref",
        "legacy_oracle_entry_id",
    }
    required_grouping = {
        "station_id",
        "radio_id",
    ...
    for index, member in enumerate(members):
        ...
        _require(
            member["recording_id"] not in recordings,
            f"{location}: duplicate recording_id",
        )
        recordings.add(member["recording_id"])
        _require(
            member["partition"]
            in {"development", "train", "validation", "locked_test"},
            f"{location}: invalid partition",
        )
        if manifest["purpose"] == "development":
            _require(
                member["partition"] != "locked_test",
        ...
        _require(
            isinstance(member["legacy_manifest_created_utc_ns"], int),
            f"{location}: UTC nanoseconds must be an integer",
        )
        grouping = member["grouping"]
        _require(
            set(grouping) == required_grouping,
            f"{location}.grouping: fields differ from v1",
        )
        _require(
            grouping["radio_id"] == member["coverage"]["radio_id"],
            f"{location}: radio differs between grouping and coverage",
        )
        _require(
            grouping["receiver_chain_ids"] == member["coverage"]["receiver_chain_ids"],
            f"{location}: receiver chains differ between grouping and coverage",
        )
        _require(
            member["split_group_id"] == split_group_id(grouping),
        ...
        coverage = member["coverage"]
        _require(
            set(coverage) == required_coverage,
            f"{location}.coverage: fields differ from v1",
        )
        _require(
            isinstance(coverage["receiver_chain_ids"], list)
            and len(coverage["receiver_chain_ids"]) == 2,
            f"{location}: exactly two receiver chains are required",
        )
        _require(
            coverage["observation_mode"]
            in {"narrow", "wide", "oversample", "channel-hop"},
            f"{location}: invalid observation mode",
        )
        _require(
            coverage["gain_mode"] in {"manual", "slow_attack"},
            f"{location}: invalid gain mode",
        )
        _require(
            isinstance(coverage["confounds"], list),
        ...
        _validate_relative_path(
            payload["base_relative_path"], f"{location}.payload_set.base_relative_path"
        )
        _validate_object_ref(
            payload["index_ref"], f"{location}.payload_set.index_ref", root_ids
        )
        _require(
            payload["index_kind"] in {"legacy_chunks_and_survey", "evidence_clips"},
            f"{location}: invalid payload index kind",
        )
        _validate_sha256(
            payload["index_sha256"], f"{location}.payload_set.index_sha256"
        )
        _require(
            isinstance(payload["object_count"], int) and payload["object_count"] > 0,
    ...
    leaked = {
        group: sorted(parts)
        for group, parts in group_partitions.items()
        if len(parts) != 1
    }
    _require(
        not leaked, f"split leakage: groups occur in multiple partitions: {leaked}"
    )
    _validate_sha256(manifest["membership_digest_sha256"], "membership_digest_sha256")
    _require(
        manifest["membership_digest_sha256"] == membership_digest(members),
    ...
    *,
    ...
    for member in manifest["members"]:
        ...
        for ref in refs:
            ...
            _verify_file(
                path, expected_bytes=ref["bytes"], expected_sha256=ref["sha256"]
            )
        ...
        _require(
            payload_index_digest(entries) == payload["index_sha256"],
            f"{member['member_id']}: payload index digest mismatch",
        )
        _require(
            len(entries) == payload["object_count"],
        ...
        if verify_payloads:
            ...
            for entry in entries:
                ...
                _require(
                    base == candidate or base in candidate.parents,
...
def validate_oracle(oracle: Mapping[str, Any], manifest: Mapping[str, Any]) -> None:
    ...
    _require(
        oracle["membership_digest_sha256"] == manifest["membership_digest_sha256"],
        "oracle membership digest mismatch",
    )
    _require(
        oracle["field_semantics"].get("scientific_truth") is False,
        "legacy oracle must explicitly deny scientific truth",
    )
    expected = {
        member["legacy_oracle_entry_id"]: member for member in manifest["members"]
    }
...
def validate_synthetic_spec(spec: Mapping[str, Any]) -> None:
    _require(
        spec.get("schema") == SYNTHETIC_SCHEMA, "unsupported synthetic fixture schema"
    )
    _require(
        spec.get("generator_contract", {}).get("detector_independent") is True,
    ...
    _require(
        layout.get("layout") == "sample,receiver,component",
    ...
    for index, case in enumerate(cases):
        ...
        _require(
            isinstance(case.get("sample_rate_hz"), int) and case["sample_rate_hz"] > 0,
            f"{location}: sample_rate_hz must be positive integer",
        )
        _require(
            isinstance(case.get("sample_count"), int) and case["sample_count"] > 0,
            f"{location}: sample_count must be positive integer",
        )
        _require(
            isinstance(case.get("seed_u64"), int) and 0 <= case["seed_u64"] < 2**64,
            f"{location}: seed_u64 out of range",
        )
        signal = case.get("signal_truth", {})
        for key in ("frequency_start_hz", "drift_hz_s", "snr_db"):
            _require(
                isinstance(signal.get(key), str),
        ...
        frequency = int(signal["phase_step_start_q64"]) * case["sample_rate_hz"] / 2**64
        drift = (
            int(signal["phase_step_delta_q64"]) * case["sample_rate_hz"] ** 2 / 2**64
        )
        _require(
            math.isclose(
                float(signal["frequency_start_hz"]),
                frequency,
                rel_tol=0.0,
                abs_tol=5e-10,
            ),
        ...
        _require(
            math.isclose(float(signal["drift_hz_s"]), drift, rel_tol=0.0, abs_tol=5e-9),
        ...
        expected_ratio = Fraction(
            ...
            2
            * int(case["noise_truth"]["uniform_component_peak_counts"])
            * (int(case["noise_truth"]["uniform_component_peak_counts"]) + 1),
        )
        _require(
            snr_ratio == expected_ratio,
        ...
        _require(
            quant.get("rounding") == "nearest_ties_away_from_zero",
            f"{location}: quantization rounding not explicit",
        )
        _require(
            isinstance(quant.get("clip_min"), int)
        ...
        _require(
            expected.get("bytes") == case["sample_count"] * 8,
            f"{location}: paired CI16 byte count mismatch",
        )
        _require(
            isinstance(expected.get("clipped_component_count"), int)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.verify_files or args.verify_payloads:
            verify_references(
                manifest, _parse_roots(args.root), verify_payloads=args.verify_payloads
            )
        gaps = coverage_gaps(manifest)
