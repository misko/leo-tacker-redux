...
CREATE TABLE public.recording_starlink_adaptive_qam_v0_4 (
  analysis_id text PRIMARY KEY CHECK(analysis_id~'^slqam4_[0-9a-f]{32}$'),
  ...
  source_adaptive_response_analysis_id text NOT NULL REFERENCES public.recording_starlink_adaptive_response_v0_1(analysis_id),
  source_adaptive_response_bundle_digest_value text NOT NULL CHECK(source_adaptive_response_bundle_digest_value~'^[0-9a-f]{64}$'),
  ...
  window_count integer NOT NULL CHECK(window_count BETWEEN stream_count AND stream_count*24),
  point_count integer NOT NULL CHECK(point_count=window_count*2400),
  idempotency_key text NOT NULL UNIQUE CHECK(idempotency_key<>''),
  published_at_utc timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),
  UNIQUE(recording_id,input_recording_digest_value,source_adaptive_response_analysis_id,source_adaptive_response_bundle_digest_value,source_suite_analysis_id,source_suite_bundle_digest_value,request_digest_value),
  FOREIGN KEY(bundle_digest_algorithm,bundle_digest_value) REFERENCES public.object_blob(digest_algorithm,digest_value)
);
CREATE INDEX recording_starlink_adaptive_qam_latest_v0_4_idx ON public.recording_starlink_adaptive_qam_v0_4(recording_id,published_at_utc DESC,analysis_id DESC);
CREATE TRIGGER recording_starlink_adaptive_qam_bundle_live BEFORE INSERT OR UPDATE OF bundle_digest_algorithm,bundle_digest_value ON public.recording_starlink_adaptive_qam_v0_4 FOR EACH ROW EXECUTE FUNCTION public.object_blob_assert_live_reference('bundle_digest_algorithm','bundle_digest_value');

CREATE FUNCTION public.publish_recording_starlink_adaptive_qam_v0_4(jsonb) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $function$
DECLARE p alias for $1; inserted text;
BEGIN
 IF jsonb_typeof(p)<>'object' OR (p->>'stream_count')::integer NOT BETWEEN 1 AND 16 OR (p->>'window_count')::integer NOT BETWEEN (p->>'stream_count')::integer AND (p->>'stream_count')::integer*24 OR (p->>'point_count')::integer<>(p->>'window_count')::integer*2400 THEN RAISE EXCEPTION 'invalid adaptive QAM publication' USING ERRCODE='22023'; END IF;
 PERFORM 1 FROM public.recording r JOIN public.recording_starlink_adaptive_response_v0_1 a ON a.recording_id=r.recording_id JOIN public.recording_starlink_detector_suite s ON s.recording_id=r.recording_id WHERE r.recording_id=p->>'recording_id' AND a.analysis_id=p->>'source_adaptive_response_analysis_id' AND a.bundle_digest_value=p->>'source_adaptive_response_bundle_digest_value' AND s.analysis_id=p->>'source_suite_analysis_id' AND s.bundle_digest_value=p->>'source_suite_bundle_digest_value' AND s.input_recording_digest_value=p->>'input_recording_digest_value';
 IF NOT FOUND THEN RAISE EXCEPTION 'adaptive QAM source closure differs' USING ERRCODE='23503'; END IF;
 INSERT INTO public.recording_starlink_adaptive_qam_v0_4 VALUES(p->>'analysis_id',p->>'recording_id',p->>'input_recording_digest_value',p->>'source_adaptive_response_analysis_id',p->>'source_adaptive_response_bundle_digest_value',p->>'source_suite_analysis_id',p->>'source_suite_bundle_digest_value',p->>'request_digest_value','sha256',p->>'bundle_digest_value',(p->>'stream_count')::integer,(p->>'window_count')::integer,(p->>'point_count')::integer,p->>'idempotency_key',DEFAULT) ON CONFLICT DO NOTHING RETURNING analysis_id INTO inserted;
 IF inserted IS NOT NULL THEN RETURN true; END IF;
 IF EXISTS(SELECT 1 FROM public.recording_starlink_adaptive_qam_v0_4 e WHERE e.analysis_id=p->>'analysis_id' AND e.recording_id=p->>'recording_id' AND e.input_recording_digest_value=p->>'input_recording_digest_value' AND e.source_adaptive_response_analysis_id=p->>'source_adaptive_response_analysis_id' AND e.source_adaptive_response_bundle_digest_value=p->>'source_adaptive_response_bundle_digest_value' AND e.source_suite_analysis_id=p->>'source_suite_analysis_id' AND e.source_suite_bundle_digest_value=p->>'source_suite_bundle_digest_value' AND e.request_digest_value=p->>'request_digest_value' AND e.bundle_digest_value=p->>'bundle_digest_value' AND e.stream_count=(p->>'stream_count')::integer AND e.window_count=(p->>'window_count')::integer AND e.point_count=(p->>'point_count')::integer AND e.idempotency_key=p->>'idempotency_key') THEN RETURN true; END IF;
 RAISE EXCEPTION 'adaptive QAM catalog identity conflict' USING ERRCODE='23505';
END $function$;

CREATE FUNCTION public.read_exact_recording_starlink_adaptive_qam_v0_4(text,text,text,text)
RETURNS TABLE(analysis_id text,recording_id text,input_recording_digest_value text,source_adaptive_response_analysis_id text,source_adaptive_response_bundle_digest_value text,source_suite_analysis_id text,source_suite_bundle_digest_value text,request_digest_value text,stream_count integer,window_count integer,point_count integer,bundle_digest_algorithm text,bundle_digest_value text,bundle_byte_count bigint,bundle_media_type text,bundle_format_id text,bundle_locator text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $function$
SELECT h.analysis_id,h.recording_id,h.input_recording_digest_value,h.source_adaptive_response_analysis_id,h.source_adaptive_response_bundle_digest_value,h.source_suite_analysis_id,h.source_suite_bundle_digest_value,h.request_digest_value,h.stream_count,h.window_count,h.point_count,h.bundle_digest_algorithm,h.bundle_digest_value,o.byte_count,o.media_type,o.format_id,o.locator FROM public.recording_starlink_adaptive_qam_v0_4 h JOIN public.object_blob o ON (o.digest_algorithm,o.digest_value)=(h.bundle_digest_algorithm,h.bundle_digest_value) WHERE h.analysis_id=$1 AND h.recording_id=$2 AND h.bundle_digest_algorithm=$3 AND h.bundle_digest_value=$4 AND o.lifecycle_state='live';$function$;

CREATE FUNCTION public.read_latest_recording_starlink_adaptive_qam_v0_4(text)
RETURNS TABLE(analysis_id text,recording_id text,input_recording_digest_value text,source_adaptive_response_analysis_id text,source_adaptive_response_bundle_digest_value text,source_suite_analysis_id text,source_suite_bundle_digest_value text,request_digest_value text,stream_count integer,window_count integer,point_count integer,bundle_digest_algorithm text,bundle_digest_value text,bundle_byte_count bigint,bundle_media_type text,bundle_format_id text,bundle_locator text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $function$
...
ALTER TABLE public.recording_starlink_adaptive_qam_v0_4 OWNER TO leo_routine_owner;
GRANT SELECT,INSERT ON public.recording_starlink_adaptive_qam_v0_4 TO leo_routine_owner;
REVOKE ALL ON public.recording_starlink_adaptive_qam_v0_4 FROM PUBLIC,leo_capture,leo_analysis,leo_dashboard,leo_maintenance;
ALTER FUNCTION public.publish_recording_starlink_adaptive_qam_v0_4(jsonb) OWNER TO leo_routine_owner;
ALTER FUNCTION public.read_exact_recording_starlink_adaptive_qam_v0_4(text,text,text,text) OWNER TO leo_routine_owner;
ALTER FUNCTION public.read_latest_recording_starlink_adaptive_qam_v0_4(text) OWNER TO leo_routine_owner;
REVOKE ALL ON FUNCTION public.publish_recording_starlink_adaptive_qam_v0_4(jsonb),public.read_exact_recording_starlink_adaptive_qam_v0_4(text,text,text,text),public.read_latest_recording_starlink_adaptive_qam_v0_4(text) FROM PUBLIC,leo_capture,leo_analysis,leo_dashboard,leo_maintenance;
GRANT EXECUTE ON FUNCTION public.publish_recording_starlink_adaptive_qam_v0_4(jsonb),public.read_exact_recording_starlink_adaptive_qam_v0_4(text,text,text,text),public.read_latest_recording_starlink_adaptive_qam_v0_4(text) TO leo_analysis;
GRANT EXECUTE ON FUNCTION public.read_exact_recording_starlink_adaptive_qam_v0_4(text,text,text,text),public.read_latest_recording_starlink_adaptive_qam_v0_4(text) TO leo_dashboard;
