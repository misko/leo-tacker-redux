...
| Legacy operational windows | Varied: 10 ms probes every 3 s with ±0.5 s/100 ms follow-up; twenty 10 ms probes spaced 6 s; or three 20 ms probes in 2 s hops | QAM: bounded evenly spaced 20 ms windows, currently 8 per stream; detector: full-dwell power prescreen with sparse exact refinement | Preserve exact accounting. The prompt timeline tiles 100% of IQ contiguously. Exact Qin/surrogate/QAM work remains a disclosed selected subset until the search is optimized; never label sparse exact coverage as full coverage. |
