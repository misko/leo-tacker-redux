...
| Dual receiver use | Candidate pairing, CFO-difference and inverse-noise combined QAM were useful | Per-RX evidence remains primary. The detail UI now adds a derived inverse-EVM combination only for two authoritative receiver assignments sharing the same recording, radio, segment, edge, exact UTC window, and published known-pilot point identities. | Keep the paired view diagnostic and visibly separate from primary per-RX evidence. Never use an old label-derived offset; no historical correction enters pairing or weighting. |
...
| Doppler | Tracks were formed only after pilot candidate epochs; J1 recovered about −4.0 kHz/s | Blind V9/V19 can follow a parallel ridge with a plausible slope. Additive V26 now derives absolute pilot frequency from the captured segment center plus acquired CFO, fits only diagnostic QAM-goodness ≥0.5 windows (top-two fallback), and compares time-overlapping blind paths through an explicit 50 kHz diagnostic gate. On J1 it recovers −3.989/−3.984 kHz/s for LNB C/D and rejects the blind paths at about 304 kHz median separation. | Keep blind and pilot-associated states separate. The 50 kHz gate and QAM-goodness fit selection are candidate diagnostics, not calibrated detection thresholds. |
