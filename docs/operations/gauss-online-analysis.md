...
Capture preflight calls `capture_campaign_analysis_safe_v1(definition_digest)`.
Active leases are accepted only when their source job is registered to this
exact campaign. An unregistered job, a different campaign's job, legacy
Starlink work, or any database uncertainty closes admission. Existing lease
tokens, generations, expiry fencing, exact-ID claims, and stage barriers remain
unchanged.
...
enabled from a sealed release containing migration 0032.
